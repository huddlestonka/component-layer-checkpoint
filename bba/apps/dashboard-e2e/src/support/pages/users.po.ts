...
export const getUserTitle = (user) =>
  cy.get(`[data-cy=user-${user.id}-item-title]`);

export const getUserDeleteBtn = (user) =>
  cy.get(`[data-cy=delete-user-${user.id}-btn]`);
...
export const completeUpdateUserForm = (user) => {
  cy.get(`[data-cy=user-form-title]`)
    .clear()
    .type(`${user.title}!!`, { delay: 20 });
  cy.get('[data-cy=user-form-save]').click();
};
